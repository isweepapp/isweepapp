...
async function loadPrizes() {
  try {
    ...
    const sections = [
      { key: 'group',    icon: 'âš½', label: 'Group Stage',    note: '(capped at £50)' },
      { key: 'knockout', icon: 'ðŸ†', label: 'Knockout Stage', note: '(capped at £50)' },
      { key: 'overall',  icon: 'ðŸ¥‡', label: 'Overall',        note: '(remainder)' },
    ];
    ...
    grid.innerHTML = sections.map(s => {
      ...
      return `<div class="prize-card">
        ...
        <div class="prize-breakdown">
          <div>ðŸ¥‡ 1st (60%)  <strong>£${p.prize1}</strong></div>
          <div>ðŸ¥ˆ 2nd (30%)  <strong>£${p.prize2}</strong></div>
          <div>ðŸ¥‰ 3rd (10%)  <strong>£${p.prize3}</strong></div>
        </div>
      </div>`;
    }).join('');
  } catch {
